...
const FallbackImage = ({ src, alt, className, fallbackNode }) => {
...
const COLLECTIONS_DATA = [
  {
    ...
    year: 1985,
    category: "Cultural",
    description: "A stamp-inspired collectible dedicated to the geometry, color, rhythm, and memory of the medina.",
    story: "Marrakech opens the WORLDMARKS series with a collectible built around walls, thresholds, carved detail, and sun-washed mineral tones. It is a tribute to one of the world's most magnetic urban identities.",
    color: "#C24A1E",
    img: "WhatsApp Image 2026-03-17 at 12.03.39.jpeg"
  },
  {
    id: "cairo",
    ...
    year: 1979,
    category: "Cultural",
    description: "A monumental collectible shaped by desert gold, dynastic scale, and architectural continuity.",
    story: "Cairo enters the atlas as a city of endurance and awe. The collectible language draws on silhouette, monumentality, and the emotional weight of civilization layered across millennia.",
    color: "#C27F1E",
    img: "WhatsApp Image 2026-03-17 at 12.02.43.jpeg"
  },
  {
    id: "rio",
    ...
    year: 2012,
    category: "Cultural",
    description: "A lush and elevated collectible shaped by mountain, sea, curve, and sacred presence.",
    story: "Rio brings a different emotional frequency to the atlas: light, elevation, tropical force, and a sense of living spectacle. The Carioca landscape is a place where nature and city become one.",
    color: "#1A5C3A",
    img: "WhatsApp Image 2026-03-17 at 11.56.14.jpeg"
  },
  {
    id: "havana",
    ...
    year: 1982,
    category: "Cultural",
    description: "A vintage-modern collectible shaped by façades, sea air, patina, and cultural rhythm.",
    story: "Havana is treated as atmosphere made collectible: color memory, urban nostalgia, and architectural charisma. A city that dances between centuries.",
    color: "#1A6E8A",
    img: "WhatsApp Image 2026-03-17 at 12.05.58.jpeg"
  },
  {
    id: "sydney",
    ...
    year: 2007,
    category: "Cultural",
    description: "A striking collectible capturing the iconic sails and deep harbor blues of a modern wonder.",
    story: "Sydney enters the atlas as a masterpiece of 20th-century architecture. The collectible reflects its soaring, shell-like structures and the interplay of light and water.",
    color: "#1A4A8A",
    icon: "🌉",
    img: null
  },
  {
    ...
    minted: 3120,
    status: "Live",
    coords: "14.3506°N 100.5615°E",
    year: 1991,
    category: "Cultural",
    description: "A vibrant collectible shaped by sacred temples, golden spires, and spiritual continuity.",
    story: "Bangkok brings a radiant energy to the atlas. The collectible honors the intricate craftsmanship, golden light, and historic temple complexes of the region.",
    color: "#8B5A1A",
    icon: "🛕",
    img: null
  }
...
const ALL_SITES = [
  // Founding 6 Cities
  { city: "Marrakech", country: "Morocco", icon: "🇲🇦", color: "#C24A1E", accent: "#E8A84B", region: "africa", type: "cultural" },
  { city: "Rio de Janeiro", country: "Brazil", icon: "🇧🇷", color: "#1A5C3A", accent: "#C8A84B", region: "latin", type: "cultural" },
  { city: "Havana", country: "Cuba", icon: "🇨🇺", color: "#1A6E8A", accent: "#C8A84B", region: "latin", type: "cultural" },
  { city: "Sydney", country: "Australia", icon: "🇦🇺", color: "#1A4A8A", accent: "#E8C8F0", region: "asia", type: "cultural" },
  { city: "Cairo", country: "Egypt", icon: "🇪🇬", color: "#C27F1E", accent: "#F5D78A", region: "arab", type: "cultural" },
  { city: "Bangkok", country: "Thailand", icon: "🇹🇭", color: "#8B5A1A", accent: "#E8C8F0", region: "asia", type: "cultural" },
...
export default function App() {
  ...
  const activeCollection = COLLECTIONS_DATA.find(c => c.id === activeCollectionId) || COLLECTIONS_DATA[0];
  const activePct = activeCollection?.editions ? Math.round((activeCollection.minted / activeCollection.editions) * 100) : 0;

  // Aggregate Data for the "Bridge Gap" Genesis Status Card
  ...
  return (
    <div className="min-h-screen bg-[#0A0A0F] text-[#F0EEE8] font-sans selection:bg-[#C8A84B]/30 overflow-x-hidden">
      ...
      <style dangerouslySetInnerHTML={{__html: `
        ...
        /* Interactive List Item Transitions */
      ...
      <section className="relative min-h-screen flex items-center justify-center pt-24 overflow-hidden">
        ...
        <div className="relative z-10 text-center w-full max-w-5xl px-6 mx-auto">
          <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full border border-[#C8A84B]/20 bg-[#C8A84B]/10 text-xs font-semibold tracking-widest uppercase text-[#C8A84B] mb-8">
            ...
            Built on Solana · Tied to UNESCO
          </div>
          ...
          <p className="text-lg md:text-xl text-[#7A7A9A] max-w-2xl mx-auto mb-10 leading-relaxed">
            Each WORLDMARKS NFT is a hand-crafted digital stamp honoring a UNESCO World Heritage Site. 
            Pure collectibles. Timeless beauty. Every mint supports heritage preservation.
          </p>
          
          <div className="flex flex-wrap justify-center gap-4 mb-16">
            <a href="#collections" className="px-8 py-3.5 rounded-xl bg-[#C8A84B] text-[#0A0A0F] font-bold tracking-wide border border-[#E8C96A] hover:bg-[#E8C96A] hover:-translate-y-0.5 hover:shadow-[0_8px_30px_rgba(200,168,75,0.3)] transition-all">
              Explore Collections
            </a>
            <button onClick={() => setWalletModalOpen(true)} className="px-8 py-3.5 rounded-xl bg-transparent text-[#F0EEE8] font-semibold tracking-wide border border-[#F0EEE8]/25 hover:border-[#F0EEE8]/50 hover:bg-[#F0EEE8]/5 transition-all">
              Connect Wallet
            </button>
          </div>
          
          {/* Floating Stamps Area - Perfectly Aligned Grid */}
          <div className="grid grid-cols-3 md:grid-cols-6 gap-3 sm:gap-4 md:gap-6 justify-items-center relative mx-auto w-full">
            {[
              { id: "rio", color: "#1A5C3A", border: "#C8A84B", img: "WhatsApp Image 2026-03-17 at 11.56.14.jpeg", name: "RIO DE JANEIRO", delay: "0s" },
              { id: "cairo", color: "#C27F1E", border: "#F5D78A", img: "WhatsApp Image 2026-03-17 at 12.02.43.jpeg", name: "CAIRO", delay: "0.4s" },
              { id: "marrakech", color: "#C24A1E", border: "#E8A84B", img: "WhatsApp Image 2026-03-17 at 12.03.39.jpeg", name: "MARRAKECH", delay: "1.2s" },
              { id: "havana", color: "#1A6E8A", border: "#C8A84B", img: "WhatsApp Image 2026-03-17 at 12.05.58.jpeg", name: "HAVANA", delay: "0.8s" },
              { id: "sydney", color: "#1A4A8A", border: "#E8C8F0", img: null, name: "SYDNEY", delay: "0.2s" },
              { id: "bangkok", color: "#8B5A1A", border: "#E8C8F0", img: null, name: "BANGKOK", delay: "0.6s" }
            ].map((stamp, i) => (
              <div 
                key={i}
      ...
      <div className="bg-[#181825] border-y border-[#C8A84B]/20 py-8">
        <div className="max-w-5xl mx-auto px-6 grid grid-cols-2 md:grid-cols-4 divide-y md:divide-y-0 md:divide-x divide-[#C8A84B]/20 text-center">
          <div className="p-4"><div className="font-display text-3xl text-[#C8A84B] mb-1">1,248</div><div className="text-xs text-[#7A7A9A] tracking-widest uppercase">UNESCO Sites</div></div>
          ...
          <div className="p-4"><div className="font-display text-3xl text-[#C8A84B] mb-1">$8,420</div><div className="text-xs text-[#7A7A9A] tracking-widest uppercase">Donated to UNESCO</div></div>
        </div>
      </div>

      {/* --- NEW INTERACTIVE COLLECTIONS SECTION --- */}
      <section id="collections" className="py-24 max-w-7xl mx-auto px-6">
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
          <div>
            ...
            <h2 className="font-display text-4xl md:text-5xl leading-[1.15]">A City Atlas Built<br/>as a Collectible Series</h2>
        ...
        <div className="grid lg:grid-cols-[0.85fr_1.15fr] gap-6 mb-6 items-start">
          ...
          <div className="flex flex-col gap-2.5">
            {COLLECTIONS_DATA.map(c => (
              ...
              >
                ...
                {/* Clamp description to exactly 2 lines to maintain predictable height */}
          ...
          {/* Right: Sticky Panel + Gap Bridge Card */}
          <div className="sticky top-[90px] flex flex-col gap-4">
             
             {/* 1. Main Interactive Preview Panel */}
               ...
               <div className="p-6">
                  ...
                  <div className="grid grid-cols-2 gap-3 mb-5">
                    ...
                    <div className="bg-[#0A0A0F] border border-[#C8A84B]/20 rounded-[10px] p-3">
                      ...
                      <div className="text-sm font-semibold">UNESCO Fund</div>
                    </div>
                    <div className="bg-[#0A0A0F] border border-[#C8A84B]/20 rounded-[10px] p-3">
                  ...
                  <div className="flex flex-wrap gap-3">
                    <button onClick={() => openMint(activeCollection?.city)} className="flex-1 py-3 px-6 rounded-full bg-[#C8A84B] text-[#0A0A0F] text-sm font-bold border border-[#E8C96A] hover:bg-[#E8C96A] transition-colors">
                      Mint This Stamp
                    </button>
             ...
             {/* 2. THE BRIDGE CARD - Fills the empty gap beautifully */}
             <div className="rounded-[20px] border border-[#C8A84B]/20 bg-[linear-gradient(135deg,rgba(200,168,75,0.08)_0%,rgba(10,10,15,1)_100%)] p-5 hidden lg:block relative overflow-hidden shadow-lg">
                <div className="flex justify-between items-end relative z-10">
                  <div>
                    ...
                    <div className="text-sm font-semibold text-[#F0EEE8] tracking-wide">{totalMinted.toLocaleString()} <span className="text-[#7A7A9A] font-normal">/ {totalEditions.toLocaleString()} Stamps Minted</span></div>
                  </div>
      ...
      <section id="impact" className="py-24 bg-[#181825] border-t border-[#C8A84B]/20">
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-start">
          ...
          <div>
            ...
            <h2 className="font-display text-4xl md:text-5xl leading-[1.15]">A Visible<br/>Contribution Layer</h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-6">
              <div className="p-4 border border-[#C8A84B]/20 rounded-[14px] bg-[#0A0A0F]">
                ...
                <div className="text-[13px] text-[#7A7A9A] leading-relaxed">A dedicated portion of primary mint revenue is allocated to the UNESCO donation pool</div>
              </div>
              <div className="p-4 border border-[#C8A84B]/20 rounded-[14px] bg-[#0A0A0F]">
        ...
        <div className="max-w-7xl mx-auto px-6 grid md:grid-cols-2 gap-16 items-center">
          <div className="order-2 md:order-1 flex justify-center">
            <div className="w-[240px] h-[240px] md:w-[280px] md:h-[280px] rounded-full pledge-badge flex items-center justify-center p-2.5">
              <div className="w-full h-full rounded-full bg-[#0A0A0F] flex flex-col items-center justify-center text-center p-6 border border-[#C8A84B]/20">
                ...
                <div className="text-[10px] text-[#7A7A9A] tracking-[0.08em] uppercase">UNESCO<br/>World Heritage<br/>Fund</div>
              </div>
            </div>
          </div>
          ...
          <div className="order-1 md:order-2">
            ...
            <p className="text-[#7A7A9A] leading-relaxed mb-4">
              ...
              is automatically directed to the UNESCO World Heritage Fund, supporting the preservation, 
        ...
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
            ...
            <div className="p-9 rounded-[20px] border border-[#C8A84B]/20 bg-[#0A0A0F] hover:border-[#C8A84B]/50 transition-colors relative">
              ...
              <p className="text-sm text-[#7A7A9A] leading-relaxed">Browse 1,248+ UNESCO World Heritage Sites across 170 countries. Filter by continent, category, or color palette. Find your place.</p>
              <div className="hidden md:block absolute right-[-28px] top-1/2 -translate-y-1/2 text-2xl text-[#C8A84B] z-10">→</div>
            </div>
            <div className="p-9 rounded-[20px] border border-[#C8A84B]/20 bg-[#0A0A0F] hover:border-[#C8A84B]/50 transition-colors relative">
              ...
              <p className="text-sm text-[#7A7A9A] leading-relaxed">Mint at 0.15 SOL. Receive a unique serialized digital stamp. A portion of every mint goes directly to UNESCO heritage preservation.</p>
            </div>
          </div>
        </div>
      </section>
          ...
          <div className="grid md:grid-cols-2 gap-16 items-center">
            ...
            <div className="mx-auto w-[320px] perspective-[800px]">
              <div className="relative w-full h-[320px] rounded-full preserve-3d transition-transform duration-[800ms] hover:rotate-y-180 cursor-pointer shadow-2xl">
                ...
                {/* FRONT WITH REAL IMAGE AND SAFE FALLBACK COMPONENT */}
                <div className="absolute inset-0 backface-hidden rounded-full border-[3px] border-[#E8A84B] overflow-hidden p-0 bg-[radial-gradient(circle,#C24A1E_0%,#8B300E_100%)] flex items-center justify-center">
                  <FallbackImage 
                    src="WhatsApp Image 2026-03-17 at 12.03.39.jpeg" 
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={idx} className="flex items-start gap-3 text-sm text-[#7A7A9A]">
                    ...
                    <span dangerouslySetInnerHTML={{__html: feature.replace("—", "</strong>—") .replace(/^([^—]+)/, '<strong>$1')}} />
                  </li>
                ))}
              </ul>
            </div>
      ...
      <section id="explorer" className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          ...
          <div className="grid grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-4">
            {visibleExplorerSites.length > 0 ? visibleExplorerSites.map((site, i) => (
              <div 
                key={i} 
          ...
          <div className="text-center mt-10">
             <button 
              ...
              >
              {remainingSites <= 0 ? 'All Sites Loaded ✓' : `Load More Sites (${remainingSites} remaining) ↓`}
             </button>
          </div>
        </div>
      </section>
      ...
      <section id="about" className="py-24 bg-[#181825]">
        <div className="max-w-7xl mx-auto px-6">
          <div>
            <div className="text-[11px] font-bold tracking-[0.18em] uppercase text-[#C8A84B] mb-2">Built to Scale</div>
            <h2 className="font-display text-4xl leading-[1.15] mb-12">Architecture for<br/>Hundreds of Thousands</h2>
          </div>
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="p-8 rounded-[20px] border border-[#C8A84B]/20 bg-[#0A0A0F]">
              ...
              <p className="text-[13px] text-[#7A7A9A] mb-5">Built for scale. Near-zero fees. Sub-second finality.</p>
              <ul className="space-y-2">
                {["Metaplex Token Metadata Standard", "Candy Machine v3 per city collection", "Compressed NFTs (cNFTs) for volume", "On-chain royalties via Metaplex", "Verified collection grouping"].map((item, i) => (
                  <li key={i} className="text-[13px] text-[#7A7A9A] flex gap-2"><span className="text-[#C8A84B]">→</span> {item}</li>
                ))}
              </ul>
            </div>
            <div className="p-8 rounded-[20px] border border-[#C8A84B]/20 bg-[#0A0A0F]">
              ...
              <p className="text-[13px] text-[#7A7A9A] mb-5">Permanent, tamper-proof asset storage.</p>
              <ul className="space-y-2">
                {["Arweave for permanent media storage", "IPFS fallback via NFT.Storage", "1:1 square artwork (2048×2048px)", "Animated variants (APNG / WebP)", "Full JSON metadata on-chain"].map((item, i) => (
                  <li key={i} className="text-[13px] text-[#7A7A9A] flex gap-2"><span className="text-[#C8A84B]">→</span> {item}</li>
                ))}
              </ul>
            </div>
            <div className="p-8 rounded-[20px] border border-[#C8A84B]/20 bg-[#0A0A0F]">
              ...
              <p className="text-[13px] text-[#7A7A9A] mb-5">Universal Solana wallet support.</p>
              <ul className="space-y-2">
                {["Phantom, Backpack, Solflare, Ledger", "Wallet Adapter standard", "Magic Eden launchpad listing", "Tensor marketplace integration", "Treasury auto-split for UNESCO"].map((item, i) => (
                  <li key={i} className="text-[13px] text-[#7A7A9A] flex gap-2"><span className="text-[#C8A84B]">→</span> {item}</li>
                ))}
              </ul>
            </div>
      ...
      <section id="faq" className="py-24">
        <div className="max-w-4xl mx-auto px-6">
          <div className="mb-10">
            ...
            <h2 className="font-display text-4xl leading-[1.15] mb-3">Clear Answers<br/>for Serious Collectors.</h2>
          ...
          <div className="flex flex-col gap-4">
            {[
              { q: "What am I collecting?", a: "Each piece is a unique digital stamp from the WORLDMARKS UNESCO Heritage series — a visual tribute to an extraordinary place. The focus is collection, cultural appreciation, and digital ownership on Solana." },
              { q: "Is there any promise beyond collecting?", a: "No. WORLDMARKS is built for collectors. There is no promise of profit, resale performance, or future utility. You are collecting a beautifully designed digital artifact — nothing more, nothing less." },
              { q: "How does the UNESCO donation work?", a: "A dedicated portion of primary mint revenue is allocated to our donation pool and transferred to heritage preservation initiatives. The platform's Impact Ledger displays total allocations and transfer history publicly — all verifiable on-chain." },
              { q: "Do I need a crypto wallet?", a: "A Solana wallet (Phantom, Backpack, Solflare) is the simplest path. We also support a beginner-friendly onboarding flow — you can browse the entire archive and collector list without connecting anything." },
              { q: "Why Solana?", a: "Solana's near-zero transaction fees (~$0.001) and sub-second finality make it the most accessible chain for a global collectibles platform. Compressed NFTs allow us to scale to hundreds of thousands of stamps without compromising collector access." }
            ].map((faq, i) => (
      ...
      <section id="roadmap" className="py-24 max-w-7xl mx-auto px-6 border-t border-[#C8A84B]/20">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-8 relative mt-8">
          {/* Horizontal connecting line - hidden on small */}
          <div className="hidden lg:block absolute top-[20px] left-[12.5%] right-[12.5%] h-px bg-[#C8A84B]/20 z-0"></div>
          
          <div className="text-center relative z-10 px-4">
            <div className="w-10 h-10 rounded-full border-2 border-[#C8A84B] bg-[#C8A84B]/10 shadow-[0_0_20px_rgba(200,168,75,0.3)] mx-auto flex items-center justify-center text-xs font-bold text-[#C8A84B] mb-4">Q2</div>
            ...
            <p className="text-xs text-[#7A7A9A] leading-relaxed">6 founding city collections<br/>Marrakech, Rio, Havana, Sydney, Cairo, Bangkok</p>
          </div>
          
          <div className="text-center relative z-10 px-4">
            <div className="w-10 h-10 rounded-full border-2 border-[#C8A84B]/20 bg-[#0A0A0F] mx-auto flex items-center justify-center text-xs font-bold text-[#7A7A9A] mb-4 transition-colors hover:border-[#C8A84B]">Q3</div>
            ...
            <p className="text-xs text-[#7A7A9A] leading-relaxed">20 new city collections<br/>Europe & Asia focus<br/>Animated stamp variants<br/>Collector passport feature</p>
          </div>
          
          <div className="text-center relative z-10 px-4">
            <div className="w-10 h-10 rounded-full border-2 border-[#C8A84B]/20 bg-[#0A0A0F] mx-auto flex items-center justify-center text-xs font-bold text-[#7A7A9A] mb-4 transition-colors hover:border-[#C8A84B]">Q4</div>
            <div className="text-[10px] text-[#C8A84B] tracking-[0.1em] uppercase mb-2">2026</div>
            <h3 className="font-display text-[15px] mb-2">Heritage Atlas</h3>
            <p className="text-xs text-[#7A7A9A] leading-relaxed">100+ city collections<br/>Natural heritage series<br/>Physical print program<br/>UNESCO partnership</p>
          </div>
          
          <div className="text-center relative z-10 px-4">
            <div className="w-10 h-10 rounded-full border-2 border-[#C8A84B]/20 bg-[#0A0A0F] mx-auto flex items-center justify-center text-xs font-bold text-[#7A7A9A] mb-4 transition-colors hover:border-[#C8A84B]">∞</div>
            ...
            <p className="text-xs text-[#7A7A9A] leading-relaxed">All 1,248 UNESCO sites<br/>Multi-language support<br/>Collector leaderboards<br/>Heritage travel rewards</p>
          </div>
        </div>
      </section>
      ...
      <footer className="bg-[#0A0A0F] border-t border-[#C8A84B]/20 pt-16 pb-8 mt-12">
        <div className="max-w-7xl mx-auto px-6 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-12 mb-12">
          <div className="lg:col-span-2">
            ...
            <p className="text-[13px] text-[#7A7A9A] leading-relaxed max-w-[260px] mb-5">
              Digital stamps honouring UNESCO World Heritage Sites. Built on Solana. 
              Pure collectibles. Every mint supports heritage preservation globally.
            </p>
            <div className="flex gap-3">
              <button className="w-9 h-9 rounded-lg border border-[#C8A84B]/20 bg-[#181825] flex items-center justify-center text-sm hover:border-[#C8A84B] hover:text-[#C8A84B] transition-colors"><Twitter className="w-4 h-4" /></button>
              <button className="w-9 h-9 rounded-lg border border-[#C8A84B]/20 bg-[#181825] flex items-center justify-center text-sm hover:border-[#C8A84B] hover:text-[#C8A84B] transition-colors"><Camera className="w-4 h-4" /></button>
              <button className="w-9 h-9 rounded-lg border border-[#C8A84B]/20 bg-[#181825] flex items-center justify-center text-sm hover:border-[#C8A84B] hover:text-[#C8A84B] transition-colors"><MessageCircle className="w-4 h-4" /></button>
            </div>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {['Collections', 'All Sites', 'Design System', 'Roadmap'].map(link => (
                <li key={link}><a href="#" className="text-[13px] text-[#7A7A9A] hover:text-[#F0EEE8] transition-colors">{link}</a></li>
              ))}
            </ul>
          </div>
          ...
          <div>
            ...
            <ul className="space-y-2.5">
              {['UNESCO Heritage', 'How to Mint', 'Solana Wallets', 'FAQs'].map(link => (
                <li key={link}><a href="#" className="text-[13px] text-[#7A7A9A] hover:text-[#F0EEE8] transition-colors">{link}</a></li>
              ))}
            </ul>
          </div>
        ...
        <div className="max-w-7xl mx-auto px-6 pt-6 border-t border-[#C8A84B]/20 flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
          ...
          <div className="text-[11px] text-[#7A7A9A] max-w-[500px] md:text-right leading-relaxed">
            WORLDMARKS NFTs are digital collectibles only. They carry no financial promises, 
            no utility guarantees, and no investment value. UNESCO is not affiliated with or endorsing this project. 
            Donations are made voluntarily by the creators.
          </div>
        </div>
      </footer>
      ...
      {/* Wallet Modal */}
      {walletModalOpen && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setWalletModalOpen(false)}>
          <div className="bg-[#181825] border border-[#C8A84B]/20 rounded-[20px] p-9 w-full max-w-[420px] relative shadow-2xl" onClick={e => e.stopPropagation()}>
            <button onClick={() => setWalletModalOpen(false)} className="absolute top-4 right-4 text-[#7A7A9A] hover:text-[#F0EEE8] transition-colors"><X className="w-5 h-5" /></button>
            ...
            <div className="space-y-2.5">
              <div onClick={connectWallet} className="flex items-center gap-3.5 p-3.5 rounded-xl border border-[#C8A84B]/20 bg-[#0A0A0F] hover:border-[#C8A84B] hover:bg-[#C8A84B]/5 transition-colors cursor-pointer group">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xl bg-[#181825] group-hover:scale-110 transition-transform">👻</div>
                <div><div className="text-sm font-semibold">Phantom</div><div className="text-[11px] text-[#7A7A9A]">Most popular</div></div>
                <div className="ml-auto text-[11px] text-[#7A7A9A] flex items-center gap-1">Detected <span className="w-1.5 h-1.5 bg-green-500 rounded-full inline-block"></span></div>
              </div>
              <div onClick={connectWallet} className="flex items-center gap-3.5 p-3.5 rounded-xl border border-[#C8A84B]/20 bg-[#0A0A0F] hover:border-[#C8A84B] hover:bg-[#C8A84B]/5 transition-colors cursor-pointer group">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xl bg-[#181825] group-hover:scale-110 transition-transform">🎒</div>
                <div><div className="text-sm font-semibold">Backpack</div><div className="text-[11px] text-[#7A7A9A]">xNFT wallet</div></div>
                <div className="ml-auto text-[11px] text-[#7A7A9A]">—</div>
              </div>
              <div onClick={connectWallet} className="flex items-center gap-3.5 p-3.5 rounded-xl border border-[#C8A84B]/20 bg-[#0A0A0F] hover:border-[#C8A84B] hover:bg-[#C8A84B]/5 transition-colors cursor-pointer group">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xl bg-[#181825] group-hover:scale-110 transition-transform">☀️</div>
                <div><div className="text-sm font-semibold">Solflare</div><div className="text-[11px] text-[#7A7A9A]">Web & mobile</div></div>
                <div className="ml-auto text-[11px] text-[#7A7A9A]">—</div>
              </div>
              <div onClick={connectWallet} className="flex items-center gap-3.5 p-3.5 rounded-xl border border-[#C8A84B]/20 bg-[#0A0A0F] hover:border-[#C8A84B] hover:bg-[#C8A84B]/5 transition-colors cursor-pointer group">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-xl bg-[#181825] group-hover:scale-110 transition-transform">🔐</div>
                <div><div className="text-sm font-semibold">Ledger</div><div className="text-[11px] text-[#7A7A9A]">Hardware wallet</div></div>
                <div className="ml-auto text-[11px] text-[#7A7A9A]">—</div>
              </div>
            </div>
          </div>
      ...
      {/* Mint Modal */}
      {mintModalOpen && (
        <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4" onClick={() => setMintModalOpen(false)}>
          <div className="bg-[#181825] border border-[#C8A84B]/20 rounded-[20px] p-9 w-full max-w-[420px] relative shadow-2xl" onClick={e => e.stopPropagation()}>
            <button onClick={() => setMintModalOpen(false)} className="absolute top-4 right-4 text-[#7A7A9A] hover:text-[#F0EEE8] transition-colors"><X className="w-5 h-5" /></button>
            <h2 className="font-display text-[22px] mb-2">Mint — {selectedCity}</h2>
            <p className="text-sm text-[#7A7A9A] mb-6">You are about to mint a unique WORLDMARKS stamp. Review details below.</p>
            ...
            <div className="bg-[#0A0A0F] border border-[#C8A84B]/20 rounded-xl p-5 mb-6">
              ...
              <div className="flex justify-between text-[13px]">
                <span className="text-[#7A7A9A]">UNESCO Donation</span>
                <span className="text-[#C8A84B] font-medium">Portion → UNESCO Fund</span>
              </div>
            </div>
            ...
            {walletConnected ? (
              ...
              <button onClick={() => { setMintModalOpen(false); setWalletModalOpen(true); }} className="w-full py-3.5 rounded-xl bg-[#C8A84B] text-[#0A0A0F] font-bold tracking-wide border border-[#E8C96A] hover:bg-[#E8C96A] hover:-translate-y-0.5 transition-all">
